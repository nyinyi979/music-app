...
type Actions = {
    setSongData: ({}:{songName: string, artist: string, duration: number|string}) => void;
...
export const useUserStore = create<UserData & Actions>((set) => ({
    ...
    setSongData: ({artist,duration,songName}:{songName: string, artist: string, duration: number|string}) => {
        set(() => ({artist,duration:Number(duration),songName,playing:true}));
    },
