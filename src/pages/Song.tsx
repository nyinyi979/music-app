...
export default function Songs() {
  ...
  React.useEffect(()=>{
    setLoading(true)
    const data = fetch(`https://www.theaudiodb.com/api/v1/json/2/track.php?m=${params.id||112024}`).then((data)=>{
      return data.json();
    })
    data.then((data)=>{
      setSongs(data.track)
  ...
  return (
    <div className="w-full h-[100vh] flex flex-col gap-6 py-10 px-10 bg-white overflow-y-auto text-black">
      <p className="font-semibold text-lg">{loading ? "" : songs[0].strAlbum }</p>
      <div className="flex flex-col gap-5">
        {loading ? new Array(10).fill(0).map((x,index)=><div key={x+index} className="w-full h-10 bg-gray-100 animate-pulse"/>) :
        songs.map((song) => (
          <button
            key={song.idTrack}
            onClick={()=>updateSong({artist:song.strArtist,duration:song.intDuration,songName:song.strTrack})}
