...
export default function RecomenedForYou() {
  ...
  React.useEffect(()=>{
    const data = fetch("https://www.theaudiodb.com/api/v1/json/2/track.php?m=2115886").then((data)=>{
      return data.json();
    })
    data.then((data)=>{
      setSongs(data.track)
        ...
        songs.map((song) => (
          <button key={song.idTrack} className="song">
            <div className="song-hover">
              <img src="https://www.theaudiodb.com/images/media/album/thumb/thursday-4ee69293cd094.jpg" className="size-full bg-gray-300" />
              <div className="play">
                <div>
                  <Play stroke="#fff" />
                </div>
